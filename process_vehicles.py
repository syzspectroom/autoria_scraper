...
def find_matching_brand(title, brands):
    ...
    for brand_name, brand_info in brands.items():
        if (
            title_words and brand_name.lower() == title_words[0]
        ) or f"{brand_name.lower()} " in title.lower():
...
def main():
    ...
    for car in cars:
        ...
        title = car["Title"]
        matching_brand, _ = find_matching_brand(title, brands)
        ...
        if matching_brand:
            ...
            for category in ["passenger_car", "bus"]:
                brand_models_file = os.path.join(
                    models_dir,
                    f"{sanitize_filename(matching_brand)}_{category}_models.json",
                )
            ...
            if not model_found:
                ...
                logging.warning(
                    f"No matching model found for: {title} (Brand: {matching_brand})"
                )
        else:
            car["Brand"] = "Unknown"
            car["Model"] = "Unknown"
    ...
    logging.info(
        f"Vehicles with known brands: {total_count - unknown_brand_count - missing_fields_count}"
    )
    ...
    logging.info(
        f"Percentage of unknown brands: {(unknown_brand_count / total_count) * 100:.2f}%"
    )
    logging.info(
        f"Percentage of unknown models: {(unknown_model_count / total_count) * 100:.2f}%"
    )

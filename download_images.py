...
def main():
    ...
    try:
        ...
        print(
            f"Batch existence check time: {existence_check_time - filtering_time:.2f} seconds"
        )
        ...
        cars_to_download = [
            ...
            if car.get("ID")
            and os.path.join(get_folder_path(car["ID"]), f"{car['ID']}.jpg")
        ...
        print(
            f"Skipped {len(cars) - len(eligible_cars)} cars with Unknown VehicleType."
        )

...
class CarSpiderSpider(scrapy.Spider):
    ...
    def parse(self, response):
        for content in response.xpath('//section[contains(@class, "ticket-item")]'):
            ...
            car["URL"] = "https://auto.ria.com" + data_div.xpath(
                "@data-link-to-view"
            ).get("")
            ...
            car["Price"] = (
                content.css("span.bold.size22.green::text").get("").replace(" ", "")
            )
            car["Mileage"] = (
                content.css("li.item-char.js-race::text")
                .get("")
                .replace(" тис. км", "000")
                .strip()
            )
            ...
            # Location extraction
            location = content.xpath(
                './/li[contains(@class, "js-location")]/i[@class="icon-location"]/following-sibling::text()'
            ).get()
            car["Location"] = location.strip() if location else None

            # Extract fuel type and engine volume
            fuel_type_nodes = content.xpath(
                './/li[i[contains(@class, "icon-fuel") or contains(@class, "icon-battery")]]/text()'
            ).extract()
            fuel_type = next(
                (node.strip() for node in fuel_type_nodes if node.strip()), None
            )
            ...
            if fuel_type:
                if "," in fuel_type:
                    ...
                    car["EngineVolume"] = (
                        fuel_parts[1].strip() if len(fuel_parts) > 1 else None
                    )
                else:
            ...
            generation_text = " ".join(
                text.strip()
                for text in content.css("div.generation ::text").getall()
                if text.strip()
            )
            if generation_text:
            ...
            # Gearbox extraction
            gearbox_nodes = content.xpath(
                './/li[i[contains(@class, "icon-akp") or contains(@class, "icon-transmission")]]/text()'
            ).extract()
            car["Gearbox"] = next(
                (node.strip() for node in gearbox_nodes if node.strip()), None
            )
            ...
            self.logger.debug(
                f"Fuel Type: {car['FuelType']}, Engine Volume: {car['EngineVolume']}"
            )

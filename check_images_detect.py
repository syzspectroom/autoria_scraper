...
def validate_image(image_path, valid_dir, invalid_dir):
    try:
        ...
        car_detected = any(
            result.boxes.cls[result.boxes.cls == 2].numel() > 0
            and result.boxes.conf[result.boxes.cls == 2].max() > 0.25
            for result in results
        )

        rel_path = os.path.relpath(image_path, "data/pictures")
        new_dir = os.path.join(
            valid_dir if car_detected else invalid_dir, os.path.dirname(rel_path)
        )
...
def process_images(
    ...
    with ThreadPoolExecutor(max_workers=20) as executor:
        futures = [
            executor.submit(validate_image, path, valid_dir, invalid_dir)
            for path in image_paths
        ]

        for future in tqdm(
            as_completed(futures), total=len(futures), desc="Validating images"
        ):
